...
const stackBase = z.object({ title: bilingualRequired, description: bilingualOptional, coherence: bilingualOptional, suggestedTiming: bilingualOptional, functionTag: z.enum(['regulate', 'activate', 'build', 'recover']), primaryLabel: z.string().min(1), supportingLabels: z.array(z.string()), level: z.enum(['essential', 'balanced', 'full']), daypart: z.enum(['morning', 'midday', 'evening']).nullable().default(null), isPremium: z.boolean(), isActive: z.boolean() });
...
export const microActionSchema = z.object({ title: bilingualRequired, effect: bilingualRequired, howTo: bilingualRequired, warning: bilingualRequired, labels: z.array(z.string()).min(1), durationMin: z.number().int().min(1).max(60), level: z.enum(['essential', 'balanced', 'full']) });

...
const stackBase = z.object({ title: bilingualRequired, description: bilingualOptional, coherence: bilingualOptional, suggestedTiming: bilingualOptional, functionTag: z.enum(['regulate', 'activate', 'build', 'recover']), primaryLabel: z.string().min(1), supportingLabels: z.array(z.string()), level: z.enum(['beginner', 'intermediate', 'advanced', 'expert']), daypart: z.enum(['morning', 'midday', 'evening']).nullable().default(null), isPremium: z.boolean(), isActive: z.boolean() });
...
export const microActionSchema = z.object({ title: bilingualRequired, effect: bilingualRequired, howTo: bilingualRequired, warning: bilingualRequired, labels: z.array(z.string()).min(1), durationMin: z.number().int().min(1).max(60), level: z.enum(['beginner', 'intermediate', 'advanced', 'expert']), defaultFunctionTag: z.enum(['regulate', 'activate', 'build', 'recover']).nullable().default(null) });
export const contextRowSchema = z.object({ microActionId: z.string().min(1), microActionTitle: bilingualRequired, functionTag: z.enum(['regulate', 'activate', 'build', 'recover']).nullable().default(null), stackSortOrder: z.number().int().min(0), priorityOrder: z.number().int().min(1), isOptional: z.boolean(), isActiveByDefault: z.boolean(), includedInMode: z.enum(['essential', 'balanced', 'full']), daypart: z.enum(['morning', 'midday', 'evening']), durationOverrideMin: z.number().int().min(1).nullable(), timingType: z.enum(['none', 'exact', 'window', 'relative', 'anchor']), startTime: timeOrNull, endTime: timeOrNull, relativeToContextId: z.string().nullable(), dependencyText: bilingualOptional, contextEffect: bilingualOptional, contextWarning: bilingualOptional, centreTime: timeOrNull, elasticityMin: z.number().int().min(0).nullable() }).superRefine((value, ctx) => {

...
// Drafts stay saveable with only a title; the full bilingual set and a daypart are required to activate.
const publishRequired = ['description', 'coherence', 'suggestedTiming'] as const;
const stackBase = z.object({ title: bilingualRequired, description: bilingualOptional, coherence: bilingualOptional, suggestedTiming: bilingualOptional, functionTag: z.enum(['regulate', 'activate', 'build', 'recover']), primaryLabel: z.string().min(1), supportingLabels: z.array(z.string()), level: z.enum(['beginner', 'intermediate', 'advanced', 'expert']), daypart: z.enum(['morning', 'midday', 'evening']).nullable().default(null), stackOrder: z.number().int().min(0).default(0), isPremium: z.boolean(), isActive: z.boolean() });
export const stackSchema = stackBase.superRefine((value, ctx) => {
  ...
  if (!value.daypart) ctx.addIssue({ code: 'custom', path: ['daypart'], message: 'Choose a daypart before activating.' });
});
// Warning is deliberately optional and never auto-filled: an empty warning lets the app omit the
// warning block entirely. Effect stays required.
export const microActionSchema = z.object({ title: bilingualRequired, effect: bilingualRequired, howTo: bilingualRequired, warning: bilingualOptional, labels: z.array(z.string()).min(1), durationMin: z.number().int().min(1).max(60), level: z.enum(['beginner', 'intermediate', 'advanced', 'expert']), defaultFunctionTag: z.enum(['regulate', 'activate', 'build', 'recover']).nullable().default(null) });
export const contextRowSchema = z.object({ microActionId: z.string().min(1), microActionTitle: bilingualRequired, functionTag: z.enum(['regulate', 'activate', 'build', 'recover']).nullable().default(null), stackSortOrder: z.number().int().min(0), priorityOrder: z.number().int().min(1), isOptional: z.boolean(), isActiveByDefault: z.boolean(), includedInMode: z.enum(['essential', 'balanced', 'full']), daypart: z.enum(['morning', 'midday', 'evening']).nullable().default(null), durationOverrideMin: z.number().int().min(1).nullable(), timingType: z.enum(['none', 'exact', 'window', 'relative', 'anchor']), startTime: timeOrNull, endTime: timeOrNull, relativeToContextId: z.string().nullable(), dependencyText: bilingualOptional, contextEffect: bilingualOptional, contextWarning: bilingualOptional, centreTime: timeOrNull, elasticityMin: z.number().int().min(0).nullable() }).superRefine((value, ctx) => {
...
export const notificationTemplateSchema = z.object({ triggerKey: z.string().regex(/^[a-z0-9_]+$/, 'Use lowercase letters, digits and underscores.'), title: bilingualRequired, body: bilingualRequired, deeplinkTarget: z.string().trim().max(200).default(''), isActive: z.boolean() });
